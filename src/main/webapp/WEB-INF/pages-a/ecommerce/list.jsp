    ...
    <div class="col-lg-12">
        <div class="line-height-50">
            <ul class="list-unstyled list-inline navigator-bar">
                ...
                <c:if test="${category != null and not empty category.title}" >
                    <c:if test="${category.parentCategory != null and not empty category.parentCategory.title and category.parentCategory.id != 160}" >
                        <li><i class="fa fa-angle-right"></i></li>
                        <li><a href="${ctx}/mall/list?category=${category.parentCategory.id}">${category.parentCategory.title}</a></li>
                    </c:if>
                    <li><i class="fa fa-angle-right"></i></li>
                    <li><a href="${ctx}/mall/list?category=${category.id}">${category.title}</a></li>
                </c:if>
            </ul>
        </div>
    </div>
...
<div class="container">
    <div class="col-lg-8 col-md-8 col-sm-8 ">
        ...
        <div class="col-lg-12 bg-white margin-bottom-20">
            ...
            <ul class="shop-item-list row list-inline nomargin">
                <c:forEach items="${sellOffers}" var="sellOffer">
                    ...
                    <div class="shop-item">
                        <div class="thumbnail">
                            ...
                            <a class="shop-item-image" href="${ctx}/mall/item-${sellOffer.id}.html">
                                <c:choose>
                                    <c:when test="${not empty sellOffer.imageUrl}">
                                        <img src="${sellOffer.imageUrl}"/>
                                    </c:when>
                                    <c:otherwise>
                                        <img src="${ctx}/images/cumtp.com/TRLOGO.jpg"/>
                                    </c:otherwise>
                                </c:choose>
                            </a>
                            <!-- /product image(s) -->

                            <%--<c:if test="${sellOffer.flagNew == true || sellOffer.flagRecommend == true}">
                            <!-- product more info -->
                            <div class="shop-item-info">
                                <c:if test="${sellOffer.flagNew == true}">
                                <span class="label label-success">新</span>
                                </c:if>
                                <c:if test="${sellOffer.flagRecommend == true}">
                        ...
                        <div class="shop-item-summary text-center">
                            <a  href="${ctx}/mall/item-${sellOffer.id}.html">
                                <div class="book_title_view" title="${sellOffer.title}">${sellOffer.title}</div>
                            </a>
                            <div>${sellOffer.author}</div>
                            <!-- price -->
                    ...
                    <ul class="pagination">
                        ...
                        <c:forEach begin="${minPage}" end="${maxPage}" var="item">
                            ...
                            <li class="${activeClass}">
                                <a href="${ctx}${categoryUrl}?category=${category.id}&page=${item}">${item}</a>
                            </li>
                        </c:forEach>
                        <li>
                            <a href="${ctx}${categoryUrl}?category=${category.id}&?page=${resultList.totalPage}">&raquo;</a>
                        </li>
                    </ul>
    ...
    <div class="col-lg-4 col-md-4 col-sm-4">
        <c:set var="tmpCategory" value="${category}"/>
        <c:if test="${empty category}">
            <c:set var="tmpCategory" value="${rootCategory}"/>
        </c:if>
        <c:if test="${not empty tmpCategory.childCategories}">
            <div class="col-lg-12 bg-white margin-bottom-30">
                <div class="row alert alert-info" style="padding: 5px 10px; margin-bottom:10px;"><!-- SUCCESS -->
                    <div class="nopadding nomargin">${tmpCategory.title} 目录</div>
                </div>
                <div>
                    <ul class="list-unstyled sub-category-list">
                        <c:forEach items="${tmpCategory.childCategories}" var="subCategoryItem">
                            <li>
                                <a href="${ctx}/mall/list?category=${subCategoryItem.id}">${subCategoryItem.title}</a>
                            </li>
                        </c:forEach>
                    </ul>
                </div>
                                ...
                                <c:forEach items="${editorItems}" var="topItem" varStatus="status">
                                    <li class="<c:if test='${status.index == 0}'>active</c:if>">
                                        <a href="#tab_a_${status.index}" data-toggle="tab">
                                            <img src="${ctx}/${topItem.imageInfo.imagePath}"
